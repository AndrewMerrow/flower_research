...
class AggregateCustomMetricStrategy(fl.server.strategy.FedAvgM):
    def aggregate_fit(
        ...
        if(ourDetect):
            ...
            saved_csv = detection_slice.to_csv('Test1_Round{}_client_models.csv'.format(str(server_round)), index=False)
...
def main():
    ...
    if(args.data == "cifar10"):
        ...
        model = utils.CNN_MNIST()
        ct = datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
        filename = "Round10_client_model_kmeans_fedemnist_test_" + str(ct) + ".txt"
        with open(filename, "w") as f:
    ...
    fl.server.start_server(
        ...
        config=fl.server.ServerConfig(num_rounds=10 if selectedDataset == "fedemnist" else 200),
        strategy=strategy,
    )
