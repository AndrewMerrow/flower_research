...
def local_outlier_factor(X, clients, offset):
  ...
  topindices = np.argsort(lof)[:int(0.1*K)] # top 50% of clients with low LOF values (likely to be benign)
  topclients = clients[topindices].values
  bottomindices = np.argsort(lof)[int(0.9*K):] # bottom 50% clients with high LOF value (discard as malicious)
